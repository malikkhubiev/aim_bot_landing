...
const quizData = [
  ...
  },
  {
    ...
    answer: 'Ответ: Нет. Врачу нужно объяснение: «Модель выделила вот этот участок на снимке с такими-то характеристиками (неровные края, высокая плотность), что с высокой вероятностью указывает на злокачественность». Дальше это можно перепроверить и, если ошибок нет, тогда ставим диагноз.'
  },
  {
    type: 'reveal',
    title: '5. Инвестиции 💸',
    question: 'Модель говорит не покупать акции. Можно ли слепо следовать предсказанию?',
    answer: 'Ответ: Нет. Разумный инвестор не следует слепо чужим указаниям. Ему нужны аргументы: "У компании высокое соотношения долга к доходу, нет стабильного потока клиентов, им один раз выделили деньги, но они не доказали, что умеют ими эффективно распоряжаться."'
  },
  {
    type: 'reveal',
    title: '6. Смещение выборки 🗽',
    question: 'Опрос 1936 года предсказал неверного победителя. Почему?',
    answer: 'Ответ: Выборка была смещена: опрашивали владельцев телефонов/подписчиков — в основном богатых, не репрезентативно. Если в дорогом отеле в Дубае 0% опрошенных едят быстрорастворимую лапшу, это не значит, что в 2025 году её больше не покупают.'
  },
  {
    type: 'choice',
...
function renderStep(container, stepIndex, onDone) {
  ...
  if (stepIndex >= quizData.length) {
    ...
    p.innerHTML = 'Хотите узнать как построить машинное обучение для компании на Уолл-стрит?<br><br>Вы пройдёте все 15 этапов в интерактивном режиме и узнаете какой стиль работы у инженера ML.';
